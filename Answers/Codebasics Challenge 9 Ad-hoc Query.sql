...
select campaign_name,
    ...
    (case when promo_type = '50% OFF' or promo_type = 'BOGOF' then base_price * 0.5 * quantity_sold_after_promo /*assuming BOGOF applies for the total quantity in the orders, not once and the quanity given for free is counted in the quantity_sold_after_promo */
...
select campaign_name, round(sum(total_revenue_before_promo)/1000000, 2) as total_revenue_before_promo, round(sum(total_revenue_after_promo)/1000000,2) as total_revenue_after_promo
from cte
...
select p.product_name,
    ...
    (1.0* base_price * quantity_sold_before_promo) as total_revenue_before_promo,
    (case when promo_type = '50% OFF' or promo_type = 'BOGOF' then base_price * 0.5 * quantity_sold_after_promo /*assuming BOGOF applies for the total quantity in the orders, not once and the quanity given for free is counted in the quantity_sold_after_promo */
    when promo_type = '25% OFF' then base_price * 0.25 * quantity_sold_after_promo
    when promo_type = '33% OFF' then base_price * 0.33 * quantity_sold_after_promo
    else base_price * quantity_sold_after_promo END) as total_revenue_after_promo
from fact_events e
inner join dim_products p on e.product_code = p.product_code
)
